...
def make_aggval_dict(aggval_csv, metric_cols, proj_ctype, yearkey, geo_regn,
    ...
    aggval_csv = CSV file of all pre-calcualted agg vals for region and community types
    metric_cols = metrics you want comm type and region values for
    proj_ctype = community type you want ctype values for
    yearkey = name of the year column in aggval CSV
    geo_regn = name of the field containing region values
    yearval = if specifying a specific year (e.g., base year vs. future year) for the same metric,
        specify which year you want the value for
    """

...
LOCAL_CONFIG = str(config.local_config_dir())
...
def safety_gate(folder_abs, exit_on_failure=True):
    hits = []
    ...
    if hits:
        message = "SAFETY GATE FAILED — prod references found in local-mode target:\n"
        for f, b in hits:
            message += f"   {f}: {b!r}\n"
        if exit_on_failure:
...
def create_private_scratch(sample_path, subreport):
    """Create a collision-free scratch folder beside this run's input evidence."""
    sample_abs = sample_path if os.path.isabs(sample_path) else os.path.join(HERE, sample_path)
    run_dir = os.path.dirname(os.path.abspath(sample_abs))
    return tempfile.mkdtemp(prefix=f"scratch_{subreport}_", dir=run_dir)
...
def main():
    ...
    sample_abs = sample if os.path.isabs(sample) else os.path.join(HERE, sample)
    with open(sample_abs) as f:
        raw = json.load(f)
    ...
    import arcpy
    # Give every service process a private scratch workspace. Never delete ArcPy's default
    # scratch.gdb: it is shared by standalone Python processes for this Windows account and may
    # be in use by ArcGIS Pro or another development session. A fresh private folder also avoids
    # the stale/non-GDB scratch condition that causes ERROR 000840.
    private_scratch = create_private_scratch(sample, subreport)
    arcpy.env.scratchWorkspace = private_scratch
    ...
    dest = os.path.join(OUT_DIR, f"{subreport}_{stamp}_{os.getpid()}.json")
    with open(result) as src, open(dest, "w") as dst:
        dst.write(src.read())
    # The result has been preserved in OUT_DIR, so remove only this invocation's private scratch.
    # It lives beside the run input and was created by create_private_scratch above; never touch
    # ArcPy's account-wide default scratch workspace.
    config.assert_within_root(private_scratch, os.path.dirname(os.path.abspath(sample_abs)))
    shutil.rmtree(private_scratch, ignore_errors=True)
    print(f"OK -> {dest}")
